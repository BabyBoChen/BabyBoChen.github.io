...
(function main() {
    ...
    loadObject3D('BabyBoGoGoGo.glb').then(function (babybo) {
        ...
        babybo.meshesToDispose.forEach(mesh => meshes.push(mesh));
        animationMixers.push(createAnimationMixer(babybo));
    });
    loadObject3D('jessy.glb').then(function (jessy) {
        scene.add(jessy.scene);
        jessy.scene.position.x += 0.5;
        rotatingGroups.push(jessy.scene);
        jessy.meshesToDispose.forEach(mesh => meshes.push(mesh));
    });
